...
if python3 -c 'import sys; raise SystemExit(sys.version_info < (3, 11))'; then
    ...
    echo "<WARNING> python3 3.11 or newer is required for SmartHomeBridge."
    exit 1
fi
...
if python3 -m venv --help >/dev/null 2>&1; then
    ...
    echo "<WARNING> python3 venv support not found. Install python3-venv before starting SmartHomeBridge."
    exit 1
fi
